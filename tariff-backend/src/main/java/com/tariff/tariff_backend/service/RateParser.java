...
public class RateParser {
    ...
    public record ParsedRate( // record basically auto generate constructor and getters with equals and hashcode and tostrings
        ...
        public boolean isFree() { // check if its 0 or if its the long words one (default to 0)
            for (RateComponent c : components) {
                ...
                if (c.kind == RateKind.AD_VALOREM && c.adValorem != null
                        && c.adValorem.compareTo(BigDecimal.ZERO) == 0) {
                    continue;
                }

                if (c.kind == RateKind.SPECIFIC_PER_UNIT && c.specificPerUnit != null
                        && c.specificPerUnit.compareTo(BigDecimal.ZERO) == 0) {
                    continue;
                }
                // if anything else
                return false;
    ...
    private static RateComponent singularParse(String splitString) { // extracting the percent/cents/dollars or wtv
        ...
        if (mc.matches()) {
            ...
            BigDecimal dollars = cents.movePointLeft(2);             // cents -> dollars
            BigDecimal perOne  = normalizePerOneUnit(dollars, unit); // handle /1000, /doz
            String unitLabel   = normalizeUnitLabel(unit);

            return new RateComponent(RateKind.SPECIFIC_PER_UNIT, BigDecimal.ZERO, perOne, unitLabel, qualifier, str);
        ...
        if (mn.matches()) {
            ...
            return new RateComponent(RateKind.AD_VALOREM, frac, BigDecimal.ZERO, null, null, str);
        }
    ...
    private static String extractQualifier(String tail) {
        ...
        if (low.startsWith("on ") || low.startsWith("of ")) {
            return s; // preserve raw, e.g. "on the battery", "of total sugars"
        }
        return null;
    }
    ...
    private static String normaliseUnit(String rawUnitToken) { // i tried to find all the cases i could and just stuff i thought it might be
        ...
        switch (unit) {
            ...
            case "unit", "piece", "each" -> {
                return "/unit";
            }
            case "gross" -> {
                return "/gross";
            }
            case "m3", "m^3", "cubicmeter", "cubicmeters" -> {
                return "/m3";
            }
            case "m2", "m^2", "squaremeter", "squaremeters" -> {
                return "/m2";
            }
            case "m", "meter", "meters" -> {
                return "/m";
    ...
    private static BigDecimal normalizePerOneUnit(BigDecimal dollars, String unitRaw) {
        if (dollars == null)
            return BigDecimal.ZERO;
        if (unitRaw == null)
            return dollars;

        // If unit is "/<number>", divide by that number so it's per-1 unit
        if (unitRaw.matches("^/\\d+$")) {
            int n = Integer.parseInt(unitRaw.substring(1));
            if (n > 0) {
                return dollars.divide(BigDecimal.valueOf(n), java.math.MathContext.DECIMAL128);
            }
        }

        if (unitRaw.matches("/thousand")){
            return dollars.divide(BigDecimal.valueOf(1000), java.math.MathContext.DECIMAL128);

        }

        // Dozen → per 1 unit
        if ("/doz".equals(unitRaw)) {
            return dollars.divide(BigDecimal.valueOf(12), java.math.MathContext.DECIMAL128);
        }

        // Otherwise already per-1 (e.g. /unit, /kg, /liter, /m)
        return dollars;
    }

    private static String normalizeUnitLabel(String unitRaw) {
        if (unitRaw == null)
            return null;
        if (unitRaw.matches("^/\\d+$"))
            return "/unit"; // after numeric normalization, call it /unit
        if ("/doz".equals(unitRaw))
            return "/unit";
        return unitRaw; // keep /kg, /liter, /unit, etc.
    }
}
