...
def prediction(ts_matrix, wa_matrices, phi, theta, prediction_lag=1):
    """
    Calculates predictions for time series matrix
    :param ts_matrix: time series matrix (T x N)
    :param wa_matrices: list of spatial adjacency matrices
    :param phi: autoregressive parameters (p x s)
    :param theta: moving average parameters (q x s)
    :param prediction_lag: number of future time steps to forecast
    :return: forecasted time steps (prediction_lag x N)
    """
    predictions = ts_matrix.copy()
    ...
    for h in range(prediction_lag):
        ...
        forecast_steps.append(predict)
        predictions = np.concatenate((predictions, predict), axis=0)
        residuals = np.concatenate((residuals, np.zeros_like(predict)), axis=0)

    return np.vstack(forecast_steps)
